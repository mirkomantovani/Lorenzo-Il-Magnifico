...
public class ResourceChest {
	...
	public ResourceChest() {
		...
		for(int i=0;i<ResourceType.values().length;i++){
			resources.put(ResourceType.values()[i],
					ResourceFactory.getResource(ResourceType.values()[i],0));
		}		
	...
	public ResourceChest(int coin, int wood, int stone, int serv, int fp, int vp, int mp) {
		...
		for(int i=0;i<ResourceType.values().length;i++){
			resources.put(ResourceType.values()[i],
					ResourceFactory.getResource(ResourceType.values()[i],res[i]));
		}		
	}
	...
	public boolean isEmpty(){
		for(int i=0;i<ResourceType.values().length;i++){
			if(this.resources.get(ResourceType.values()[i]).getAmount()!=0)return false;
		}
		return true;
	}
	...
	public void subChest(ResourceChest resourceChest){
		...
		for(int i=0;i<ResourceType.values().length;i++){
			this.resources.get(ResourceType.values()[i])
			.sub(resourceChest.resources.get(ResourceType.values()[i]));
		}
	...
	public void addChest(ResourceChest resourceChest){
		for(int i=0;i<ResourceType.values().length;i++){
			this.resources.get(ResourceType.values()[i])
			.add(resourceChest.resources.get(ResourceType.values()[i]));
		}
	}
	...
	public boolean isGreaterEqualThan(ResourceChest resourceChest) {
		...
		for(int i=0;i<ResourceType.values().length;i++){
			if(this.resources.get(ResourceType.values()[i]).getAmount()
			<resourceChest.resources.get(ResourceType.values()[i]).getAmount())return false;
		}
		return true;
